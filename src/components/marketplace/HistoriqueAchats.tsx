...
function mapHistoryItem(item: BuyerHistoryItem): Achat {
  return {
    id: item.id,
    ...
    vendeur: item.vendeur ?? item.partenaire ?? "Vendeur",
...
export function HistoriqueAchats() {
  ...
  const {
    ...
    queryKey: ["marketplace", "orders", "buyer", "history", "achat", { searchQuery, statusFilter, dateFilter, page }],
    queryFn: () => ordersApi.getBuyerHistory({
      type: "achat",
      status: statusFilter,
      period: dateFilter,
  ...
  const achats = (historyPage?.data ?? []).map(mapHistoryItem);
  const kpiAchats = historyPage?.kpis.achats;
  const enCours = achats.filter(a => a.status === "pending" || a.status === "shipped").length;

  const handleViewDetail = (achat: Achat) => {
    setSelectedAchat(achat);
    setShowDetailDialog(true);
    ordersApi.getTransactionDetail(achat.id, "achat")
      .then((full) => setSelectedAchat(mapHistoryItem(full)))
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Consultez l'historique de vos achats sur la marketplace
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xl font-bold">{kpiAchats?.count ?? 0}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xl font-bold">{(kpiAchats?.montantFormattedM ?? 0).toFixed(1)}M</p>
        ...
        <CardContent className="p-4">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Rechercher par référence, produit, vendeur..."
                className="pl-10"
                value={searchQuery}
      ...
      {isLoading ? (
        <Card><CardContent className="py-12 flex items-center justify-center text-muted-foreground">
          <RefreshCw className="w-5 h-5 animate-spin mr-2" /> Chargement des achats...
        </CardContent></Card>
      ) : isError ? (
        <Card><CardContent className="py-12 text-center">
          <AlertTriangle className="w-10 h-10 mx-auto mb-3 text-destructive opacity-70" />
          <p className="text-sm text-muted-foreground mb-4">Impossible de charger l'historique des achats.</p>
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="font-semibold mb-2">Aucun achat</h3>
            <p className="text-sm text-muted-foreground">
        ...
        <Card>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {achats.map((a) => {
                  const status = statusConfig[a.status] ?? defaultStatus;
                    ...
                    <TableRow key={a.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <span>{a.imageProduit}</span>
                          <span className="truncate max-w-[150px]">{a.produit}</span>
                        </div>
                      </TableCell>
                      <TableCell className="truncate max-w-[150px]">{a.vendeur}</TableCell>
                      <TableCell>{a.date ? new Date(a.date).toLocaleDateString('fr-FR') : "—"}</TableCell>
                      ...
                      <TableCell>
                        <Button variant="ghost" size="icon" onClick={() => handleViewDetail(a)}>
      ...
      <Dialog open={showDetailDialog} onOpenChange={setShowDetailDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Achat du{" "}
              {selectedAchat?.date && new Date(selectedAchat.date).toLocaleDateString('fr-FR')}
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedAchat && (
            <div className="space-y-6 py-4">
              ...
              <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/50">
                <span className="text-4xl">{selectedAchat.imageProduit}</span>
                <div className="flex-1">
                  <h3 className="font-semibold">{selectedAchat.produit}</h3>
                  <p className="text-sm text-muted-foreground">
                    {selectedAchat.quantite} x {selectedAchat.prixUnitaire.toLocaleString()} FCFA
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-lg font-bold">{selectedAchat.montantTotal.toLocaleString()} FCFA</p>
                  <Badge className={cn(
                    (statusConfig[selectedAchat.status] ?? defaultStatus).color,
                    "bg-transparent border-0"
                  )}>
                    {(statusConfig[selectedAchat.status] ?? defaultStatus).label}
                  </Badge>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-medium flex items-center gap-2">
                    ...
                    {selectedAchat.livraison}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm text-muted-foreground">Paiement</p>
                  <p className="font-medium">{selectedAchat.paiement}</p>
                </div>
              </div>
